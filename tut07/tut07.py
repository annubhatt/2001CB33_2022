from asyncore import read										# Importing the required libraries
...
def find_octant(a,b,c):                                         #  initializing a Function to find the octant 
...
def octant_analysis(mod=5000):
	...
	def process_file(s,mod):										#  defining a Function to process individual file
		in_file='input/'+s											#  this is the Path to file
		df=read_excel(in_file)										# Reading file in dataframe
		wb=Workbook()												# now we are Starting a new workbook as wb
		...
		def octant_range_names(mod=5000):
			...
			dic={}                                                          #  now we are creating dictionary for mapping 
			...
			count=[0]*9                                                     #  now we are Resetting the values in list 'count'
